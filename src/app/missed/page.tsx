...
export default function MissedPage() {
  ...
  const ownedCodes = useMemo(() => new Set(userStickers.map((s) => s.code)), [userStickers]);
  const totalMissed = useMemo(() => ALL_STICKERS.filter((code) => !ownedCodes.has(code)).length, [ownedCodes]);

  // Agrupar por grupo e depois por seleção
  const groupedData = useMemo(() => {
    const groups: { groupLabel: string; teams: { id: string; name: string; missed: string[] }[] }[] = [];

    // Especiais primeiro
    const specialTeams = STICKERS_DATA.filter((t) => !t.group)
      .map((team) => ({ id: team.id, name: team.name, missed: team.stickers.filter((c) => !ownedCodes.has(c)) }))
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-10">
          <h1 className="text-4xl font-black tracking-tight text-white mb-2">Figurinhas que Faltam</h1>
          <p className="text-slate-400">{totalMissed} figurinha{totalMissed !== 1 ? "s" : ""} para completar a coleção</p>
        </div>

        {totalMissed === 0 ? (
          <div className="rounded-3xl border-2 border-dashed border-white/10 py-24 text-center">
            <div className="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-white/5 text-secondary">
              <Star size={40} />
            </div>
            <h2 className="text-2xl font-bold text-white mb-2">Coleção completa! 🎉</h2>
            <p className="text-slate-500">Parabéns! Você possui todas as figurinhas do álbum.</p>
          </div>
        ) : (
                    ...
                    <section key={team.id}>
                      ...
                      <div className="flex items-center gap-3 mb-4 p-3 rounded-xl bg-white/5 border border-white/10">
                        ...
                        <span className="text-xs font-black text-red-400 bg-red-400/10 px-2 py-1 rounded-full">
                          {team.missed.length} faltando
                        </span>
                      </div>

                      <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10">
                        {team.missed.map((code) => (
                          <StickerCard
                            key={code}
                            code={code}
                            count={0}

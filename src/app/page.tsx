...
import { STICKERS_DATA, ALL_STICKERS, GROUPS, ISO_MAP, EMBLEM_MAP, PLAYER_DATA_MAP } from "@/data/stickers";
import { Trophy, CheckCircle2, Circle, Search, ChevronRight, Link as LinkIcon } from "lucide-react";
...
export default function Dashboard() {
  ...
  const [selectedTeamId, setSelectedTeamId] = useState(STICKERS_DATA.find(t => t.group === "A")?.id || "BRA");
  ...
  const handleStickerAction = async (code: string, action: "toggle" | "increment" | "decrement") => {
    ...
    try {
      await fetch("/album-copa/api/stickers", {
        method: "POST",
  ...
  const stats = useMemo(() => {
    ...
    const repeated = userStickers.reduce((acc, curr) => acc + (curr.quantity - 1), 0);
  ...
  const copyPublicLink = () => {
    if (!userId) return;
    const url = typeof window !== 'undefined' ? `${window.location.origin}/album-copa/public/${userId}` : '';
      ...
      <main className="container mx-auto px-4 py-8">
        <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="animate-in fade-in slide-in-from-left duration-700">
            ...
            <p className="text-slate-400">Controle sua coleção oficial Panini</p>
          </div>
        </header>
        ...
        <section className="mb-12 grid grid-cols-2 gap-4 md:grid-cols-4 md:gap-6">
          <div className="rounded-2xl bg-white/5 p-4 md:p-6 border border-white/10 backdrop-blur-sm">
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div>
                <p className="text-[10px] md:text-sm font-medium text-slate-400">Progresso</p>
                <p className="text-xl md:text-2xl font-bold">{stats.percentage}%</p>
              </div>
            </div>
          ...
          <div className="rounded-2xl bg-white/5 p-4 md:p-6 border border-white/10 backdrop-blur-sm shadow-lg shadow-green-500/5">
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div>
                <p className="text-[10px] md:text-sm font-medium text-slate-400">Possui</p>
                <p className="text-xl md:text-2xl font-bold">{stats.uniqueOwned}</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-white/5 p-4 md:p-6 border border-white/10 backdrop-blur-sm">
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div>
                <p className="text-[10px] md:text-sm font-medium text-slate-400">Faltam</p>
          ...
          <div className="rounded-2xl bg-white/5 p-4 md:p-6 border border-white/10 backdrop-blur-sm shadow-lg shadow-secondary/5 relative flex flex-col justify-between min-h-[120px]">
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div>
                <p className="text-[10px] md:text-sm font-medium text-slate-400">Repetidas</p>
                <p className="text-xl md:text-2xl font-bold">{stats.repeated}</p>
              </div>
            </div>
        ...
        <section className="space-y-8">
              ...
              {currentTeams.map((team) => {
                ...
                return (
                  ...
                  >
                    ...
                    {selectedTeamId === team.id && !search && <ChevronRight size={16} className="text-secondary" />}
                  </button>
                );
              })}
            </div>
          ...
          {filteredStickers.length === 0 && (
            <div className="py-20 text-center">
              <p className="text-slate-500 text-lg">Nenhuma figurinha encontrada.</p>
            </div>
          )}
        </section>
      </main>
